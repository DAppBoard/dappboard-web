...
module.exports = {
  ...
  fn: async function (inputs, exits) {
    ...
    var base = knex.select('value AS nft').where('token_address', inputs['address']).where('timestamp', '>=', inputs['startDate']).where('timestamp', '<=', inputs['endDate']).count('* as transfers').from('token_transfers').groupBy('value');
    ...
    var base2 = knex2.where('token_address', inputs['address']).where('timestamp', '>=', inputs['startDate']).where('timestamp', '<=', inputs['endDate']).countDistinct('value as total').count('* as total_transfers').from('token_transfers');
    for (var col in inputs.query) {
      if (inputs.query.hasOwnProperty(col)) {
